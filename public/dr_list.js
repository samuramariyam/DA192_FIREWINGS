...
request.open('GET', 'http://localhost:5000/apitut-5987c/us-central1/app/api/docters/'+ui, true);
...
function AddShed(){
   ...
   did=m.get("btn21");
    ...
    for(var i=1;i<=14;i++){
       n[i] = document.getElementById(i).value; 
           ...
           {
             case 1:day='asun';
                        addtt(n[i],n[i+7],uid,did,sid,day);
                        ti=addTime(n[i],n[i+7]);
                         break;
             case 2:day='bmon';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             case 3:day='ctue';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             case 4:day='dwed';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             case 5:day='ethu';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             case 6:day='ffri';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             case 7:day='gsat';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
             default:day='none';ti=addTime(n[i],n[i+7]); addtt(n[i],n[i+7],uid,did,sid,day);break;
           }
  ...
  function addtt(tf,tt,u,did,sid,day){
       
            // Creating a XHR object 
            let xhr = new XMLHttpRequest(); 
            let url = "http://localhost:5000/apitut-5987c/us-central1/app/api/addtt"; 
            ...
            xhr.onreadystatechange = function () { 
             ...
             //var times=firebase.database.ServerValue.TIMESTAMP
            // Converting JSON data to string 
            var data = JSON.stringify({ "timefrom":tf,"timeto":tt,"day":day,"uid":u,"did":did,"sid":sid}); 
